...
```sh
  ls -s /opt/tomcat/bin/startup.sh /usr/local/bin/tomcatup
  ls -s /opt/tomcat/bin/shutdown.sh /usr/local/bin/tomcatdown
  tomcatup
```
#### check point :
...
Update users information in the /opt/tomcat/webapps/manager/META-INF/context.xml file
